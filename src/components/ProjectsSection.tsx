...
const projects = [
  ...
  },
  {
    ...
    description:
      "A self-hosted, multi-tenant error tracking platform (Sentry/Bugsnag-style) that lets developers monitor errors and events across multiple web applications. A lightweight JavaScript/TypeScript SDK auto-captures uncaught errors, failed fetch/XHR requests, and manual logs, parses raw stack traces into structured frames with source-code context, fingerprints errors for grouping, and transmits batched, retry-safe payloads with offline localStorage fallback. The Express backend ingests logs via per-project API keys, groups thousands of identical errors into single issues using reservoir sampling, tracks affected users and browser sessions, and streams new logs live to the dashboard over Server-Sent Events. The React dashboard delivers a multi-view experience — overview analytics with time-series trend charts, grouped issue management with status workflows and regression detection, a user explorer, and a Sentry-like interactive error detail view with expandable stack frame cards. Backed by JWT auth, MongoDB time-series collections, and a dynamically-bucketed analytics aggregation layer.",
    techs: [
      "TypeScript",
      "Node.js",
      "Express",
